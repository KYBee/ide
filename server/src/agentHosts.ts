...
import type { AgentHostConfig, AgentType, HostSkillRegistry, SessionKind, SessionStatus, SessionSummary, SkillRegistry, TmuxWindowsResponse } from "./types.js";
...
interface CreateRemoteSessionResult {
  id?: string;
  name?: string;
  type?: SessionKind;
}
...
export async function listAgentHostSkills(): Promise<HostSkillRegistry> {
  const hosts = listAgentHosts();
  const results = await Promise.allSettled(
    hosts.map(async (host) => {
...
export async function createAgentSession(
  ...
  input: { name: string; type: SessionKind; agentType?: AgentType; cwd?: string; command?: string }
): Promise<{ id: string }> {
  const result = await requestAgent<CreateRemoteSessionResult>(host, "/api/sessions", {
    method: "POST",
    body: JSON.stringify(input)
  });
  const tmuxName = result.name ?? result.id?.replace(/^tmux:/, "") ?? input.name;
  return { id: `tmux:${host.id}:${tmuxName}` };
}
