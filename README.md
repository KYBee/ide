...
왼쪽 세션 목록의 동그라미는 tmux 세션 단위 상태를 나타냅니다. Codex, Claude, Gemini 같은 agent 세션은 세션 안의 모든 window/pane 화면을 agent별 adapter로 확인한 뒤 하나의 상태로 집계합니다. Shell 세션은 별도 화면 판별 없이 입력 가능한 상태로 봅니다.

- 초록색: 입력 가능한 대기 상태이거나 작업이 정상적으로 실행 중입니다.
- 노란색: 하나 이상의 pane이 일반 입력이나 선택을 기다립니다.
- 주황색: 하나 이상의 pane이 승인 또는 권한 확인을 기다립니다.
- 빨간색: 하나 이상의 pane에서 에러가 감지되었습니다.
- 회색: 완료, 알 수 없음, 또는 상태를 확정하기 어려운 경우입니다.

집계 우선순위는 `error > needs_approval > waiting_input > running > completed > idle`입니다. 따라서 여러 pane 중 하나라도 입력 대기, 승인 대기, 에러 상태면 세션 동그라미가 그 상태 색상으로 표시됩니다. `running`과 `idle`은 모두 정상 상태로 보고 초록색으로 표시합니다.
