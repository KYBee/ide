...
- 초록색: 모든 pane이 입력 가능한 대기 상태입니다.
- 파란색: 하나 이상의 pane이 작업 중이거나 실행 중입니다.
...
집계 우선순위는 `error > needs_approval > waiting_input > running > completed > idle`입니다. 따라서 여러 pane 중 하나라도 승인 대기나 에러 상태면 세션 동그라미가 그 상태 색상으로 표시됩니다.
